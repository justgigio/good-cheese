...
from src.models import Boleto, BoletoFile
...
class BoletoService:
    ...
    def upload_boleto(boleto_file: BoletoFile, file_contents: bytes):
        ...
        file_chunks = [
            file_lines[pos : pos + chunk_size]
            for pos in range(0, len(file_lines), chunk_size)
        ]
        ...
        for chunk in file_chunks:
            qu.put_nowait(
                lambda cursor: BoletoService._insert_file_chunck_on_db(
                    chunk, columns, boleto_file.id, cursor
                )
            )
            chunk_lines += len(chunk)

        print("#" * 70)
        print(f"LINES: {len(file_lines)}")
        ...
        t2 = time()
        print("=" * 50)
        print(f"Inserted... {t2 - t1}s")
        print(f"Total {t2 - t0}s")

    @staticmethod
    def _insert_file_chunck_on_db(
        file_chunk: List[str], columns: Tuple, boleto_file_id: int, cursor: DBAPICursor
    ):
    ...
    def check_upload_boleto(boleto_file_id: int) -> Dict[str, bool | int | float]:
        ...
        boleto_count = (
            session.query(Boleto).filter_by(boleto_file_id=boleto_file.id).count()
        )
    ...
    def get_boletos_to_send() -> List[Row[Tuple]]:
        ...
        condition = and_(
            Boleto.processed_at == None, Boleto.debt_due_date < due_date_offset
        )
        boletos = (
            session.query(
                Boleto.id,
    ...
    @staticmethod
    def send_boleto_email(
        boleto: Dict[str, int | str], boleto_attachment: bytes, callback: Callable
    ):
